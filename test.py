...
# We use df_test which contains the original 'Hour', 'Weekday', 'DateTime'
# and join it with comparison_df which has the predictions
# They share the same index, so we can use .join()
...
# Make sure Weekday is in the right order
...
fig, axes = plt.subplots(3, 2, figsize=(18, 18)) # Made figure wider
...
# --- IMPORTANT: Ensure this code block runs *after* model_co and model_c6h6 are fitted ---

print("\n--- Step 10: Saving Models to Disk ---")
# Define the filenames for the two models
model_co_filename = 'catboost_co_model.pkl'
model_c6h6_filename = 'catboost_c6h6_model.pkl'

# Save the CO model
try:
    joblib.dump(model_co, model_co_filename)
    print(f"✅ CO model saved successfully to: {model_co_filename}")
except NameError:
    # This handles the case where the variable 'model_co' wasn't defined (i.e., the training failed)
    print("❌ ERROR: 'model_co' object not found. Did the training in Step 6 complete successfully?")

# Save the C6H6 model
try:
    joblib.dump(model_c6h6, model_c6h6_filename)
    print(f"✅ C6H6 model saved successfully to: {model_c6h6_filename}")
except NameError:
    print("❌ ERROR: 'model_c6h6' object not found. Did the training in Step 6 complete successfully?")